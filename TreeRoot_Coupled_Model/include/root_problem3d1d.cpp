...
void 
root_problem3d1d::assembly_nonlinear_mat(size_type iter)
{
...
////////// Solve the problem ///////////////////////////////////////////    
bool//
root_problem3d1d::solve_pass(size_type iter)
{
	#ifdef M3D1D_VERBOSE_
...
bool  
root_problem3d1d::solve(void)
{
 
	#ifdef M3D1D_VERBOSE_
	cout << "Allocating Non linear Term: NLtt..." << endl;
	#endif
	gmm::resize(NLtt, dof.Ut(),dof.Ut()); gmm::clear(NLtt);

	#ifdef M3D1D_VERBOSE_
	cout << "Allocating Auxiliar Term CM,Pt_old..." << endl;
	#endif

	gmm::resize(CM, dof.tot(),dof.tot());  gmm::clear(CM);
	gmm::resize(CFM,dof.tot());			   gmm::clear(CFM);
	gmm::resize(Pt_old,dof.Pt());          gmm::clear(Pt_old);
	...
	#ifdef M3D1D_VERBOSE_
	cout<<"Assembling non linear tissue matrix"<<endl<<"   For the first iteration is assumed as saturated problem"<<endl;
	#endif
	
	assembly_nonlinear_mat(N_iter);
	...
	while( Error>minERR  && N_iter<Max_iter && !Saturated ) {   
		...
		N_iter++;
		gmm::clear(Pt_old);	
		gmm::copy(gmm::sub_vector(UM,gmm::sub_interval( dof.Ut(), dof.Pt() ) ), Pt_old);
	}
	...
	if((Error>minERR )&&(Max_iter>1 )&& !Saturated)
	{
		...
		#endif
		solved= false;
	} else if(Max_iter>1 && !Saturated){  
		#ifdef M3D1D_VERBOSE_
		cout<<"*---------------------------------------------------------------------------*"<<endl;
		cout<<"* The Fpoint ";
		cout<<"Method converged in "<<N_iter<<" iterations with L2: error "<<Error<<"   *"<<endl;
