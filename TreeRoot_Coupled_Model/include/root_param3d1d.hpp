...
struct root_param3d1d : public param3d1d {
	...
	scalar_type Gamma_;
	// Costa for retentioin of Water
	scalar_type A_ret_;
	// Esponent for retention of Water
	...
	// Z Position in the Porous Medium
	vector_type Z_;
			...
			) 
	{
		...
		Z_.resize(dof_datat); //-------------------------------------------NB RHO g l'ho già tenuto conto nel vettore Z------
		bgeot::base_node bn;//------------------------------------------------ tenuto conto in k_t--------------
		for(size_type ind=0; ind<dof_datat; ind++){
			bn=mf_datat_.point_of_basic_dof(ind);
			Z_[ind]=bn[2]*g_*rho_;
		}
		// Check values
	...
	//! Get the Z position vector
	vector_type & Z (void){return Z_;}
